...
def train(args):
    ...
    if idr_torch.rank == 0:
        ...
        run = wandb.init(project="audioset", \
                   entity="dcls", \
                   name=wandb_run_name)
    ...
    train_indexes_hdf5_path = os.path.join(dataspace, 'hdf5s', 'indexes', 
        '{}.h5'.format(data_type))

    eval_bal_indexes_hdf5_path = os.path.join(dataspace, 
    ...
    if "Deformable" in model_type :
        ...
        model = convnext_base(pretrained=True, strict=False, in_22k=in_22k, drop_path_rate=drop_path_rate, after_stem_dim=after_stem_dim,
                               use_speed_perturb=use_speed_perturb,
                               use_pydub_augment=use_pydub_augment,
                               use_roll_augment=use_roll_augment)
        print("Pretrained: True")
        ...
        print("Pretrained: True")
        model = convnext_nano(pretrained=True, strict=False, drop_path_rate=drop_path_rate, after_stem_dim=after_stem_dim,
                              use_speed_perturb=use_speed_perturb,
                              use_pydub_augment=use_pydub_augment,
                              use_roll_augment=use_roll_augment)
    else:
    ...
    scheduler = optim.lr_scheduler.OneCycleLR(optimizer, max_lr=learning_rate, total_steps=75000)
    # scheduler = optim.lr_scheduler.OneCycleLR(optimizer, max_lr=learning_rate, total_steps=early_stop)

    wd_schedule_values = None
    if use_wd_scheduler:
        def wd_scheduler(base_value, final_value, min_value, niter_per_ep, cooldown_steps=0, constant_cooldown=False):
            ...
            if constant_cooldown:
                ...
                schedule = np.array(
                    [min_value + 0.5 * (base_value - min_value) * (1 + math.cos(math.pi * i / (len(iters)))) for i in iters])

            start_warmup_value = schedule[-1]
            warmup_schedule = np.linspace(start_warmup_value, final_value, niter_per_ep-cooldown_steps)
        ...
        wd_schedule_values = wd_scheduler(base_value=weight_decay,
        ...
        print("Max WD = %.7f, Min WD = %.7f constant_cooldown=True" % (max(wd_schedule_values), min(wd_schedule_values)))
    ...
    for batch_data_dict in train_loader:
        ...
        if (iteration % 5000 == 0 and iteration > resume_iteration) or iteration == early_stop :
            ...
            if do_log and iteration > resume_iteration and idr_torch.rank == 0:
                ...
                print('--- Iteration: {}, train time: {:.3f} s / 2000 iteration ---'\
            ...
            val_logs = {"balanced_train_mAP": bal_mAP,
                        "test_mAP": test_mAP,
        ...
        if do_log and idr_torch.rank == 0:
            if wd_schedule_values is not None:
                run.log({"lr": current_lr[0], "weight_decay": param_group["weight_decay"], "steps": iteration})
            else:
